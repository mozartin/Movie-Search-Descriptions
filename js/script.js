          ...
          $.ajax({
            ...
            success: function (data) {
              ...
              if ($(".movie-block").eq(i).hasClass("col-lg-6")) {
                ...
                $(`.one-movie:eq(${i}) div:first-child`).after(
                  "<div id='fullInfo'></div>"
                );
              ...
              function createTableRow(value, property) {
                $("#tableInfo").append(
                  `<tr class='my-tr'><td class='my-td'><b>${value}</b></td><td class='my-td'>${property}</td></tr>`
                );
              }

              let el = document.getElementsByClassName("movie-block");
              var elDistanceToTop =
                window.pageYOffset + el[i].getBoundingClientRect().top;
